...
Эта операция доступна в верхнем меню модуля NextGIS Connect: |button_newfolder|.
...
.. figure:: _static/create_group_ru_2.png
   :align: center
...
Для этого выберите в панели Connect группу, внутри которой хотите создать слой. Затем в меню панели нажмите на стрелочку вниз рядом с изображением папки |button_newfolder| и выберите |button_c_new_layer| "Векторный слой NextGIS Web".

.. figure:: _static/ngc_create_ngw_layer_panel_ru.png
...
Другой способ - в верхнем меню "Слой" выберите :menuselection:`Создать слой ‣ Новый векторный слой NextGIS Web`.
...
.. figure:: _static/ngc_create_ngw_layer_set_ru_2.png
...
* Родитель - слой создаётся внутри группы ресурсов, выделенной в дереве NextGIS Connect;
* Наименование слоя
* Тип геометрии;
...
* Поля слоя: введите отображаемое наименование и ключ, выберите тип поля, затем нажмите **Добавить в список полей**. Для каждого поля можно указать параметры: 
  ...
  * Обязательно;
  * Атрибут наименование - значение из этого поля будет использоваться как название объекта при поиске на карте.

* Можно выбрать: добавить слой в проект QGIS (или только создать его в Веб ГИС).

Также в настройках создаваемого слоя можно настроить `версионирование <https://docs.nextgis.ru/docs_ngweb/source/version.html#vers-qgis>`_. Для этого перейдите во вторую вкладку в верхней части диалогового окна. Доступны три варианта: авто - используется соответствующая `настройка Веб ГИС <https://docs.nextgis.ru/docs_ngweb/source/version.html#vers-ngw-default>`_, включено, выключено.

.. figure:: _static/ngc_create_ngw_layer_set_version_ru_2.png
   :name: ngc_create_ngw_layer_set_version_pic
   :align: center
   :width: 14cm

   Настройки версионирования для нового слоя
...
* Выберите в дереве ресурсов Веб ГИС в окне модуля NextGIS Connect один или несколько ресурсов, которые вы хотите удалить;
* Выберите пункт **Удалить** в контекстном меню.

Появится окно, показывающее общее количесто ресурсов, которые будут удалены, включая дочерние.

.. figure:: _static/ngc_delete_confirm_ru.png
   :name: ngc_delete_confirm_pic
   :align: center
   :width: 12cm

   Подтверждение удаления ресурсов

Проверьте показанный список ресурсов, и если вас всё устраивает, нажмите **Удалить**. Ресурсы исчезнут из дерева Веб ГИС в панели Connect.
