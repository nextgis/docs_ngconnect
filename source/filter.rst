...
Помимо названия, можно фильтровать дерево ресурсов по другим параметрам (список см. ниже), при помощи выражений  с условиями ``=, IN, ILIKE, LIKE``. Логические операторы ``AND`` и ``OR``  можно использовать несколько раз в одном выражении, но не комбинировать их между собой. 
...
* ``@parent`` - идентификационный номер ресурса-родителя, будут показаны все дочерние ресурсы (например, все ресурсы в конкретной папке или все стили и формы определённого слоя). Чтобы отфильтровать все уровни вниз: содержимое всех подпапок, стили всех слоёв и т.п., используйте параметр @root
* ``@root`` - идентификационный номер ресурса, будут показаны все дочерние ресурсы, их дочерние ресурсы и далее вглубь до конца, например, при выборе группы ресурсов - подпапки, слои, стили и формы слоёв.

.. hint:: 

   Введите @owner = me, чтобы найти все ресурсы, владельцем которых являетесь вы (пользователь, под которым вы авторизованы в соединении с Веб ГИС).

.. figure:: _static/ngc_filter_parent_root_ru.png
   :name: ngc_filter_parent_root_pic
   :align: center
   :width: 20cm

   Поиск веб-карт внутри папки с ID=41 при помощи параметров @parent и @root

Значения перечисленных выше параметров записываются цифрами, без кавычек. `Как узнать эти номера? <https://docs.nextgis.ru/docs_ngconnect/source/filter.html#ngc-find-id>`_
...
.. todo:: ``@description`` - описание ресурса
...
Чтобы не вводить ключи метаданных вручную, можно задать их в `настройках <https://docs.nextgis.ru/docs_ngconnect/source/ngc_settings.html#ngc-set-search>`_ и затем выбирать из списка.
