...
В открывшемся окне нажмите кнопку **Новое**:
...
В разделе "Аутентификация" выберите, как вы будете подключаться к Веб ГИС:
...
* `Добавить новую конфигурацию <https://docs.nextgis.ru/docs_ngconnect/source/ngc_install.html#new-config>`_ (т.е. данные пользователя, под которыми вы получаете доступ к Веб ГИС);
* Выбрать из существующих (после того, как вы добавили несколько разных вариантов, можно выбирать нужный из списка, в списке отражается название соединения и имя пользователя, а также тип аутентификации).
...
Добавление новой конфигурации аутентификации
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. important:: Если при регистрации на nextgis.com вы воспользовались опцией "Войти через Google", то для работы в модуле Connect нужно задать дополнительный пароль. Для этого перейдите `по ссылке <https://my.nextgis.com/password/reset/?email=>`_ и следуйте инструкциям. В личный кабинет и другие онлайн-сервисы NextGIS вы по-прежнему сможете заходить через аккаунт Google.

Для того, чтобы **добавить новую конфигурацию**, в настройках модуля нажмите кнопку с зеленым плюсом.
...
Нажмите **Сохранить** в окне "Создание подключения" (:numref:`create_connection_pic`) и затем кнопку **ОК**. 
