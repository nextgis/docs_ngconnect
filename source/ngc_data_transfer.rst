...
.. figure:: _static/NGConnect_import_menu_ru_3.png
...
* В открывшемся диалоговом окне укажите название новой Группы ресурсов, в которую будет загружен проект (по умолчанию используется название проекта). Также вы можете снять галочку "Создать веб-карту". По умолчанию при добавлении проекта целиком создаётся веб-карта со всеми его слоями.

.. figure:: _static/NGConnect_import_name_ru_3.png
...
2) если выбрана опция "создавать веб-карту" - автоматически созданная `Веб-карта <https://docs.nextgis.ru/docs_ngweb/source/webmaps_client.html#ngw-webmaps-client>`_ с заданным охватом, на которую будут добавлены все импортированные слои с учетом их группировки, иерархии и видимости в панели слоёв QGIS.

* После успешной загрузки данных вам будет предложено заменить слои в проекте на синхронизируемые слои Веб ГИС. Если выбрать этот вариант, то в дальнейшем при редактировании проекта изменения в данных будут автоматически подгружаться в Веб ГИС.

.. figure:: _static/ngc_replace_dialog_ru.png
   :name: ngc_replace_dialog_pic
   :align: center
   :width: 22cm

   Замена локальных слоёв на синхронизируемые слои Веб ГИС

.. figure:: _static/ngc_replace_result_ru.png
   :name: ngc_replace_result_pic
   :align: center
   :width: 10cm

   Слои в проекте синхронизируются с Веб ГИС

.. note:: 
	После завершения создания веб-карты она автоматически откроется в браузере. Также бстро перейти к Веб-карте можно, нажав кнопку **Открыть карту в браузере** на панели инструментов модуля или выбрав соответствующую команду в контекстном меню Веб-карты.

.. figure:: _static/NGConnect_import_view_ru_4.png
   :name: NGConnect_import_view_pic
   :align: center
   :width: 22cm
   
   Открытие веб-карты через контекстное меню
...
* После успешной загрузки данных вам будет предложено заменить слой в проекте на синхронизируемый слой Веб ГИС. Если выбрать этот вариант, то в дальнейшем при редактировании проекта изменения в данных будут автоматически подгружаться в Веб ГИС.
